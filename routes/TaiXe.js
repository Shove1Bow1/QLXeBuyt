...
const {TaiXe}=require("../service/collections/TaiXe");
const { messageRespone } = require("../ultis/messageRespone");
router.get("/all",async (req,res)=>{
    await TaiXe.find({isDelete:false},(err,result)=>{
        if(err){
            console.log(err)
            return;
        }
...
router.get("/by/:id",async (req,res)=>{
    await TaiXe.find({_id:req.params.id, isDelete:false},(err,result)=>{
        if(err){
            console.log(err)
            return;
        }
...
router.post("/update",async (req,res)=>{
    ...
    var doc =await TaiXe.updateOne({_id:req.headers.id,isDelete:false},{...req.body},(err)=>{
        if(err){
            console.log(err)
            return;
        }
...
router.delete("/delete",async (req,res)=>{
    ...
    await TaiXe.updateOne({_id:id},{isDelete:true},(err)=>{
        if(err){
            console.log(err)
            res.send(messageRespone("400"));
            return;
...
router.post("/create", async(req,res)=>{
    ...
    var doc=await TaiXe.create({...req.body},(err)=>{
        if(err)
            console.log(err);
    })
    await doc.save();
